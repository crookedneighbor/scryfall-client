...
import { cardFixture, listOfCardsFixture } from "Fixtures";
...
const fakeCard: CardApiResponse = cardFixture;
const fakeList: ListApiResponse = listOfCardsFixture;
...
describe("scryfallClient", () => {
  ...
  describe("Card object", () => {
    ...
    it("can check legality of a card", () => {
      return client.search("format:standard -format:pauper").then((list) => {
        const card = list[0];
