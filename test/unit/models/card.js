...
describe.only("Card", function () {
  ...
  it("normalizes card faces", function () {
    ...
    const cardWithFaces = wrapScryfallResponse(
      this.fixtures.cardWithTransformLayout,
      {
        requestMethod: this.fakeRequestMethod,
      }
    );

    expect(cardWithoutFaces.card_faces).to.not.equal(
      this.fixtures.card.card_faces
    );
    ...
    expect(cardWithFaces.card_faces).to.equal(
      this.fixtures.cardWithTransformLayout.card_faces
    );
  });
    ...
    it("gets rulings for card", function () {
      return this.card.getRulings().then((rulings) => {
        ...
        expect(this.fakeRequestMethod).to.be.calledWith(
          this.fixtures.card.rulings_uri
        );
        expect(rulings.data[0].comment).to.equal(
          this.fixtures.listOfRulings.data[0].comment
        );
      });
    });
  });
  ...
  describe("getPrints", function () {
    beforeEach(function () {
      this.fakeRequestMethod.resolves(
        wrapScryfallResponse(this.fixtures.listOfPrints, {
          requestMethod: this.fakeRequestMethod,
        })
      );
      this.card = wrapScryfallResponse(this.fixtures.card, {
    ...
    it("gets prints for card", function () {
      return this.card.getPrints().then((prints) => {
        ...
        expect(this.fakeRequestMethod).to.be.calledWith(
          this.fixtures.card.prints_search_uri
        );
        expect(prints[0].name).to.equal(
          this.fixtures.listOfPrints.data[0].name
        );
  ...
  describe("getImage", function () {
    ...
    it("uses default face for transform card", function () {
      ...
      expect(img).to.be.a("string");
      expect(img).to.equal(
        this.fixtures.cardWithTransformLayout.card_faces[0].image_uris.normal
      );
    });
  ...
  describe("getBackImage", function () {
    ...
    it("uses back face for transform card", function () {
      ...
      const img = card.getBackImage();
      expect(img).to.equal(
        this.fixtures.cardWithTransformLayout.card_faces[1].image_uris.normal
      );
    });
    ...
    it("can specify size for back face for transform card", function () {
      ...
      expect(img).to.equal(
        this.fixtures.cardWithTransformLayout.card_faces[1].image_uris.small
      );
    });
  ...
  describe("getTokens", function () {
    beforeEach(function () {
      this.fakeRequestMethod.onCall(0).resolves(
        wrapScryfallResponse(this.fixtures.tokens.elephant, {
          requestMethod: this.fakeRequestMethod,
        })
      );
      this.fakeRequestMethod.onCall(1).resolves(
        wrapScryfallResponse(this.fixtures.tokens.wolf, {
          requestMethod: this.fakeRequestMethod,
        })
      );
      this.fakeRequestMethod.onCall(2).resolves(
        wrapScryfallResponse(this.fixtures.tokens.snake, {
          requestMethod: this.fakeRequestMethod,
        })
      );
    });
