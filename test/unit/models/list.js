  ...
  describe("next", function () {
    beforeEach(function () {
      this.fakeRequestMethod.resolves(
        wrapScryfallResponse(this.fixtures.listOfCardsPage2, {
          requestMethod: this.fakeRequestMethod,
        })
      );
    });
    ...
    it("makes a request for the next page", function () {
      ...
      return list.next().then((list2) => {
        expect(this.fakeRequestMethod).to.have.been.calledWith(
          this.fixtures.listOfCards.next_page
        );
        expect(list2[0].name).to.equal(
          this.fixtures.listOfCardsPage2.data[0].name
        );
      });
    });
